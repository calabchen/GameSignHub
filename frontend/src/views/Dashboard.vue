...
async function handleSignAll() {
  ...
  } finally { signAllLoading.value = false }
}
</script>
...
<template>
  <div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
      <div>
        <h2 style="margin:0;font-size:18px">控制面板</h2>
        <p style="margin:4px 0 0;color:#909399;font-size:13px">{{ plugins.length }} 个游戏社区</p>
      </div>
      <el-button type="primary" :loading="signAllLoading" :disabled="credentials.length === 0" @click="handleSignAll">全部签到</el-button>
    </div>
        ...
        <el-card shadow="hover">
          ...
          <div style="display:flex;gap:10px">
            <div v-for="g in p.supported_games" :key="g.id" :title="g.name" style="display:flex;flex-direction:column;align-items:center;gap:4px">
              ...
              <span style="font-size:10px;color:#909399">{{ g.name }}</span>
            </div>
          </div>
        </el-card>
      </el-col>
