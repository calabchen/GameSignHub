<script setup lang="ts">
  ...
  function resolveGameId(account: any): string | null {
    if (Array.isArray(account.enabled_games) && account.enabled_games.length > 0) {
      return account.enabled_games[0]
    }
    const plugin = plugins.value.find((p: any) => p.id === account.plugin_id)
    const games = plugin?.supported_games || []
    return games.length > 0 ? games[0].id : null
  }

  async function handleSign(account: any) {
    const accountId = account.id
    const gameId = resolveGameId(account)
    if (!gameId) {
      ElMessage.warning('未找到可签到的游戏')
      return
    }

    signLoading.value = accountId
    try {
      await signCredential(account.plugin_id, accountId, gameId)
  ...
  onMounted(async () => {
    ...
    try {
      allAccounts.value = await fetchCredentials()
    } catch { }
    filterPlugin.value = (route.query.plugin as string) || ''
  })

  const filteredAccounts = () => {
    if (!filterPlugin.value) return allAccounts.value
    return allAccounts.value.filter((a: any) => a.plugin_id === filterPlugin.value)
  }
  ...
  function openEdit(account: any) {
    ...
    form.id = account.id
    form.plugin_id = account.plugin_id
    form.display_name = account.display_name
    form.credentials_json = ''
    form.enabled_games = [...(account.enabled_games || [])]
  ...
  <div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
      <div style="display:flex;align-items:center;gap:16px">
        <h2 style="margin:0;font-size:20px">账户管理</h2>
        ...
        <span style="color:#909399;font-size:13px">{{ filteredAccounts().length }} 个账户</span>
      </div>
    ...
    <el-table :data="filteredAccounts()" stripe empty-text="暂无数据">
      ...
      <el-table-column label="操作" width="290" fixed="right">
        <template #default="{ row }">
          <el-button size="small" type="primary" :loading="signLoading === row.id"
            @click="handleSign(row)">签到</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <el-form label-width="80px">
        ...
        <el-form-item label="名称">
          <el-input v-model="form.display_name" placeholder="例如：主账号" />
        </el-form-item>
        <el-form-item label="账户内容">
          <el-input v-model="form.credentials_json" type="textarea" :rows="4" placeholder='{"token":"你的token"}' />
        ...
        <el-form-item label="游戏">
          <el-checkbox-group v-model="form.enabled_games">
            <el-checkbox v-for="g in (plugins.find(p => p.id === form.plugin_id)?.supported_games || [])" :key="g.id"
              :label="g.id" :value="g.id">{{ g.name || g.id }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
